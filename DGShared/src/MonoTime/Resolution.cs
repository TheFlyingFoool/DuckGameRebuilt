...
namespace DuckGame
{
    public class Resolution
    {
        ...
        public static void Initialize(object pWindow, GraphicsDeviceManager pDeviceManager)
        {
            ...
            DevConsole.Log(DCSection.General, "Default adapter size is (" + dims.Item1.ToString() + "x" + dims.Item2.ToString() + ")");
            ...
            {
                dimensions = new Vec2(dims.Item1, dims.Item2)
            }, false);
...
#if __ANDROID__
            var dims = MainActivity.GetDimensions();
            if (pResolution.x > dims.Item1 || pResolution.y > dims.Item2)
#else
