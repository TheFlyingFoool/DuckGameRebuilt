...
namespace DuckGame
{
    public class UnlockTree : Thing
    {
        ...
        public override void Update()
        {
            ...
            {
                UnlockData unlockData3 = (UnlockData)null;
                UnlockData unlockData4 = (UnlockData)null;
        ...
        public override void Draw()
        {
            ...
            for (int index1 = 0; index1 < treeLayer.Count && num4 < num2; ++index1)
            {
                ...
                if (index1 == treeLayer.Count - 1 && collection.Count > 0)
                {
                    for (int index2 = 0; index2 < treeLayer.Count; ++index2)
                    {
                        ...
                        if (unlockData2.children.Count > 0)
                        {
                            ...
                            if (!unlockData2.ProfileUnlocked(Profiles.active[0]))
                            {
                                ...
                                color3 = Color.Green;
                            color3 = new Color((byte)((double)color3.r * (double)num7), (byte)((double)color3.g * (double)num7), (byte)((double)color3.b * (double)num7));
                            for (int index3 = 0; index3 < collection.Count; ++index3)
                            {
                                ...
                                {
                                    float num9 = collection.Count != 1 ? (collection.Count != 2 ? (float)index3 * (vec2_2.x / (float)(collection.Count - 1)) : (float)((double)vec2_2.x / 2.0 - (double)vec2_2.x / 4.0 + (double)index3 * ((double)vec2_2.x / 2.0))) : vec2_2.x / 2f;
                    ...
                    treeLayer.Clear();
                    treeLayer.AddRange((IEnumerable<UnlockData>)collection);
                    collection.Clear();
                    ++num4;
                    index1 = -1;
                }
