...
namespace DuckGame
{
    public class Vote
    {
        ...
        public static bool Passed(VoteType type)
        {
            ...
            IEnumerable<Profile> source = Profiles.all.Where<Profile>((Func<Profile, bool>)(x => x.team != null && x.slotType != SlotType.Spectator));
        ...
        public static void Update()
        {
            ...
            foreach (RegisteredVote vote in Vote._votes)
            {
                ...
                vote.wobbleInc += 0.5f;
                if (!vote.open && (double)vote.slide < 0.00999999977648258)
        ...
        public static void Draw()
        {
            ...
            foreach (RegisteredVote vote in Vote._votes)
            {
                if (vote.who != null && vote.who.inputProfile != null)
                {
                    ...
                    Graphics.Draw((Sprite)vote.who.persona.skipSprite, (float)((double)Layer.HUD.width + (double)num5 - (double)vote.slide * 48.0 + (double)vec2.x * (double)num4) + num3, (float)((double)Layer.HUD.height - 28.0 - (double)(num1 * 16) - (double)vec2.y * (double)num4), (Depth)0.9f);
                    vote.who.persona.skipSprite.frame = 1;
                    ...
                    Graphics.Draw((Sprite)vote.who.persona.skipSprite, (float)((double)Layer.HUD.width + 68.0 - (double)vote.slide * 48.0 + (double)p2.x * 20.0) + num3, (float)((double)Layer.HUD.height - 32.0 - (double)(num1 * 16) - (double)p2.y * 20.0), (Depth)0.9f);
                    ++num1;
                }
            }
        }
