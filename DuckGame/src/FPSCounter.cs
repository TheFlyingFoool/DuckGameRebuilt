...
namespace DuckGame
{
    internal static class FPSCounter
    {
        ...
        private static void Initialize(GraphicsDevice graphics)
        {
            ...
            _biosFont = new BitmapFont(Texture2D.FromStream(_graphics, new MemoryStream(buffer)), 8);
        ...
        public static int GetFPS(int index) => _fpsValue.ContainsKey(index) ? _fpsValue[index] : 0;
          ...
          string label = "FPS")
        {
            ...
            if (_fpsValue.ContainsKey(index))
                num = _fpsValue[index];
            _batch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None, RasterizerState.CullNone, null, (Microsoft.Xna.Framework.Matrix)Matrix.Identity);
            _biosFont.Draw(_batch, Convert.ToString(num, CultureInfo.InvariantCulture) + " " + label, xpos + 1, ypos + 1, bgcolor);
            _biosFont.Draw(_batch, Convert.ToString(num, CultureInfo.InvariantCulture) + " " + label, xpos - 1, ypos - 1, bgcolor);
            _biosFont.Draw(_batch, Convert.ToString(num, CultureInfo.InvariantCulture) + " " + label, xpos - 1, ypos + 1, bgcolor);
            _biosFont.Draw(_batch, Convert.ToString(num, CultureInfo.InvariantCulture) + " " + label, xpos + 1, ypos - 1, bgcolor);
            _biosFont.Draw(_batch, Convert.ToString(num, CultureInfo.InvariantCulture) + " " + label, xpos, ypos, color);
            _batch.End();
        }
