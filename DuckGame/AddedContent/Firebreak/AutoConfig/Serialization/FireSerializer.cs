...
namespace DuckGame
{
    public static class FireSerializer
    {
        ...
        public static object Deserialize(Type type, string str)
        {
            ...
            if (type.InheritsFrom(typeof(IEnumerable))
                   && HandleIEnumerableDeserialize(type, str, out IEnumerable result))
                return result;
        ...
        private static bool HandleIEnumerableDeserialize(Type enumerableType, string from, out IEnumerable result)
        {
            ...
            IEnumerable<object> usableCollection = split.Select(x => Deserialize(argType, UnescapeString(x)));

            Array arr = Array.CreateInstance(argType!, split.Length);
            ...
            {
                Type listType = typeof(List<>).MakeGenericType(argType!);
