...
namespace DuckGame
{
    public class MessageDialogue : ContextMenu
    {
        ...
        public override void Update()
        {
            ...
            float num1 = 300f;
            float num2 = 80f;
            Vec2 vec2_1 = new Vec2((float)(this.layer.width / 2.0 - num1 / 2.0), (float)(this.layer.height / 2.0 - num2 / 2.0) + this.windowYOffsetAdd);
            Vec2 vec2_2 = new Vec2(vec2_1.x + 18f, this.bottomRightPos - 50f);
            Vec2 vec2_3 = new Vec2(120f, 40f);
            float num3 = (float)((this.layer.width / 2.0 + num1 / 2.0 - vec2_1.x) / 2.0);
            if (this.okayOnly)
                vec2_2 = new Vec2((float)(vec2_1.x + num3 - vec2_3.x / 2.0), this.bottomRightPos - 50f);
            ...
            switch (Editor.inputMode)
            {
                case EditorInput.Mouse:
                    this._hoverOk = Mouse.x > vec2_2.x && Mouse.x < vec2_2.x + vec2_3.x && Mouse.y > vec2_2.y && Mouse.y < vec2_2.y + vec2_3.y;
                    if (!this.okayOnly)
                    {
                        this._hoverCancel = Mouse.x > vec2_4.x && Mouse.x < vec2_4.x + vec2_5.x && Mouse.y > vec2_4.y && Mouse.y < vec2_4.y + vec2_5.y;
                        break;
                    }
                    break;
        ...
        public override void Draw()
        {
            ...
            float num1 = 300f;
            float num2 = 80f;
            Vec2 p1_1 = new Vec2((float)(this.layer.width / 2.0 - num1 / 2.0), (float)(this.layer.height / 2.0 - num2 / 2.0) + this.windowYOffsetAdd);
            Vec2 p2 = new Vec2((float)(this.layer.width / 2.0 + num1 / 2.0), (float)(this.layer.height / 2.0 + num2 / 2.0) + this.windowYOffsetAdd);
            ...
            if (this.okayOnly)
            {
                ...
                Vec2 p1_2 = new Vec2((float)(this.x + num3 - vec2.x / 2.0 - 2.0), p2.y - 50f);
                Graphics.DrawRect(p1_2, p1_2 + vec2, this._hoverOk ? new Color(80, 80, 80) : new Color(30, 30, 30), this.depth + 2);
                this._font.Draw("OK", (float)(p1_2.x + vec2.x / 2.0 - this._font.GetWidth("OK") / 2.0), p1_2.y + 12f, Color.White, this.depth + 3);
            }
            else
            {
                ...
                this._font.Draw("OK", (float)(p1_3.x + vec2_1.x / 2.0 - this._font.GetWidth("OK") / 2.0), p1_3.y + 12f, Color.White, this.depth + 3);
                ...
                this._font.Draw("CANCEL", (float)(p1_4.x + vec2_2.x / 2.0 - this._font.GetWidth("CANCEL") / 2.0), p1_4.y + 12f, Color.White, this.depth + 3);
            }
