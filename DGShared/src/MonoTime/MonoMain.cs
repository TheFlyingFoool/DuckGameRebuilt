...
namespace DuckGame
{
    public class MonoMain : Game
    {
        ...
        public MonoMain()
        {
...
#if __ANDROID__
            var dims = MainActivity.GetDimensions();
            _adapterW = dims.Item1;
            _adapterH = dims.Item2;
#else
            ...
            Options.PostLoad();
#if __ANDROID__
            Options.LocalData.currentResolution = new Resolution() { dimensions = new Vec2( _adapterW, _adapterH) };
#endif
            if (noFullscreen)
            ...
            if (!_started) 
            {
                ...
                if (!DuckGame.Content.didsetbigboi && Program.shouldusespriteatlas)
                {
...
#if __ANDROID__ //FIXME
                    DevConsole.Log("loading ../spriteatlass.png");
                    DuckGame.Content.Thick = (Tex2D)DuckGame.Content.SpriteAtlasTextureFromStream(  Android.OS.Environment.ExternalStorageDirectory + @"/Content/spriteatlas.png", Graphics.device);
                    DuckGame.Content.Thick.Namebase = "SpriteAtlas";

                    //RSplit("de mo", ' ', -1);
                    StreamReader stream = new StreamReader(TitleContainer.OpenStream(@"Content/spriteatlas_offsets.txt"));
                    ...
                    foreach (string line2 in lines)
                    {
                        try
                        {
                            List<string> texturedetails = DuckGame.Content.RSplit(line2, ' ', 4);
