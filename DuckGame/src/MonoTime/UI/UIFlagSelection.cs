...
namespace DuckGame
{
    public class UIFlagSelection : UIMenu
    {
        ...
        {
            Sprite flag = null;
            if (smallVersion && _sprites.TryGetValue(idx, out flag))
                return flag;
            try
            {
                ...
                if (_flagTexture != null)
                {
                    ...
                    Color[] data = new Color[_flagTexture.width * _flagTexture.height];
                    (_flagTexture.nativeObject as Texture2D).GetData(data);
                    int num3 = num1 * 61;
                    int num4 = num2 * 41;
                    for (int index1 = 0; index1 < 41; ++index1)
                    {
                        for (int index2 = 0; index2 < 61; ++index2)
                            colors[index2 + index1 * 61] = data[num3 + index2 + (num4 + index1) * _flagTexture.width];
                    }
                    tex.SetData(colors);
                    flag = new Sprite(tex);
                    _sprites[idx] = flag;
        ...
        public override void Draw()
        {
            ...
            for (int index = 0; index < numFlags; ++index)
            {
                int num7 = index % 16;
                int num8 = index / 16;
                Graphics.Draw(_flagTexture, new Vec2(num5 + num3, num6 + num4), new Rectangle?(new Rectangle(num7 * 61, num8 * 41, 61f, 41f)), num2 == _flagSelection ? Color.White : Color.White * 0.7f, 0f, Vec2.Zero, new Vec2(0.14f, 0.14f), SpriteEffects.None, (Depth)0.9f);
                num3 += 9f;
                ++num1;
                ++num2;
