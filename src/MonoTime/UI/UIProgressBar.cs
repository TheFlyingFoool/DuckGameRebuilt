...
namespace DuckGame
{
    public class UIProgressBar : UIComponent
    {
        ...
        public override void Draw()
        {
            ...
            {
                Vec2 p1 = this.position - new Vec2(this.halfWidth, num2 / 2f) + new Vec2((float)(index * (int)Math.Round((double)num1 / (double)num3)), 0.0f);
                Vec2 p2 = this.position - new Vec2(this.halfWidth, (float)(-(double)num2 / 2.0)) + new Vec2((float)((index + 1) * (int)Math.Round((double)num1 / (double)num3)) - 1f, 0.0f);
