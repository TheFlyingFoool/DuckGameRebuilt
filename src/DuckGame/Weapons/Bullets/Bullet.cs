...
namespace DuckGame
{
    public class Bullet : Thing
    {
          ...
          : base()
        {
            ...
            if (!this.traced)
            {
                ...
                if (this._gravityAffected)
                {
                    ...
                    this._physicalBullet.bullet = this;
            ...
            if (PewPewLaser.inFire)
                return;
            double x = (double)this.travelDirNormalized.x;
        }
        ...
        public Bullet ReverseTravel()
        {
            ...
            Vec2 vec2 = new Vec2((float)-Math.Sign(this.travelDirNormalized.x), 0.0f);
            ...
            do
            {
                ...
                for (int index1 = 0; index1 < 2; ++index1)
                {
                    ...
                    for (int index2 = 0; index2 < Bullet.bulletImpactList.Count; ++index2)
                    {
                        ...
                        if (flag1 == bulletImpact is IAmADuck && (bulletImpact != this._owner && (!(this._owner is Duck) || !(this._owner as Duck).ExtendsTo((Thing)bulletImpact)) || this.ammo.immediatelyDeadly) && (owner == null || bulletImpact != owner.holdObject) && bulletImpact != this._teleporter && (!(bulletImpact is Teleporter) || !this._tracer && this.ammo.canTeleport) && (this.ammo.ownerSafety <= 0 || (double)this._travelTime / (double)Maths.IncFrameTimer() >= (double)this.ammo.ownerSafety || this.firedFrom == null || bulletImpact != this.firedFrom.owner))
                        {
                            bool flag2 = false;
                            if (DevConsole.shieldMode && bulletImpact is Duck && (double)(bulletImpact as Duck)._shieldCharge > 0.600000023841858)
                            {
                                flag2 = true;
                                willBeStopped = true;
                            }
                            ...
                            if (!flag2 && (double)bulletImpact.thickness >= 0.0 && !this._currentlyImpacting.Contains(bulletImpact))
                            {
                                if (!this._tracer && !this._tracePhase)
                                {
                                    ...
                                    {
                                        ImpactedFrom from = (double)this.currentTravel.y < (double)bulletImpact.top + 1.0 || (double)this.currentTravel.y > (double)bulletImpact.bottom - 1.0 ? ((double)this.travelDirNormalized.y > 0.0 ? ImpactedFrom.Top : ImpactedFrom.Bottom) : ((double)this.travelDirNormalized.x > 0.0 ? ImpactedFrom.Left : ImpactedFrom.Right);
                                        this._physicalBullet.position = this.currentTravel;
                                        this._physicalBullet.velocity = this.velocity;
                                        if (bulletImpact is Block || bulletImpact is IPlatform && (double)this.travelDirNormalized.y > 0.0)
                                            bulletImpact.SolidImpact((MaterialThing)this._physicalBullet, from);
                                        else if ((double)bulletImpact.thickness > (double)this.ammo.penetration)
                                            bulletImpact.Impact((MaterialThing)this._physicalBullet, from, false);
                                        this.velocity = this._physicalBullet.velocity;
                                        willBeStopped = (double)bulletImpact.thickness > (double)this.ammo.penetration;
                                    }
                                    else
                                        willBeStopped = (double)bulletImpact.thickness > (double)this.ammo.penetration;
                                    ...
                                    willBeStopped = (double)bulletImpact.thickness > (double)this.ammo.penetration;
                                this.OnCollide(this.currentTravel, (Thing)bulletImpact, willBeStopped);
                                this._currentlyImpacting.Add(bulletImpact);
                                if ((double)bulletImpact.thickness > 1.5 && (double)this.ammo.penetration >= (double)bulletImpact.thickness)
                                {
                                    ...
                                    if (this._teleporter.link != null)
                                    {
                                        ...
                                        if ((double)rng > 0.0)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    if ((double)currentTravel.x < (double)this._teleporter._link.left + 2.0)
                                                        currentTravel.x = this._teleporter._link.left + 2f;
                                                    if ((double)currentTravel.x > (double)this._teleporter._link.right - 2.0)
                                                        currentTravel.x = this._teleporter._link.right - 2f;
                                                }
                                                else
                                                {
                                                    if ((double)currentTravel.y < (double)this._teleporter._link.top + 2.0)
                                                        currentTravel.y = this._teleporter._link.top + 2f;
                                                    if ((double)currentTravel.y > (double)this._teleporter._link.bottom - 2.0)
                                ...
                                else if (!flag3 && (this.rebound && (!this.ammo.softRebound || bulletImpact.physicsMaterial != PhysicsMaterial.Wood) && bulletImpact is Block || this.reboundOnce))
                                {
                                    ...
                                    if ((double)length1 > 2.0)
                                    {
                                        ...
                                        if ((double)rng > 0.0)
                                        {
                                            ...
                                            float num2 = 0.0f;
                                            float num3 = 999.9f;
                                            if ((double)this.currentTravel.y >= (double)bulletImpact.top && (double)vec2_2.y < (double)bulletImpact.top)
                                            {
                                            ...
                                            if ((double)this.currentTravel.y <= (double)bulletImpact.bottom && (double)vec2_2.y > (double)bulletImpact.bottom)
                                            {
                                                num2 = Math.Abs(this.currentTravel.y - vec2_2.y);
                                                if ((double)num2 < (double)num3)
                                                {
                                            ...
                                            if ((double)this.currentTravel.x >= (double)bulletImpact.left && (double)vec2_2.x < (double)bulletImpact.left)
                                            {
                                            ...
                                            float num4;
                                            if ((double)this.currentTravel.x <= (double)bulletImpact.right && (double)vec2_2.x > (double)bulletImpact.right)
                                            {
                                                num2 = Math.Abs(this.currentTravel.x - vec2_2.x);
                                                if ((double)num2 < (double)num3)
                                                {
                                            ...
                                            this.lastReboundSource = (Thing)bulletImpact;
                                            if (this.reboundOnce)
                ...
                Vec2 currentTravel1 = this.currentTravel;
