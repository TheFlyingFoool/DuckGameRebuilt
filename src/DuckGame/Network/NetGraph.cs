...
namespace DuckGame
{
    public class NetGraph
    {
        ...
        public void InitializeDraw(GraphicsDevice device)
        {
            ...
            NetGraph._biosFont = new NetGraph.BFont(Texture2D.FromStream(device, (Stream)new MemoryStream(buffer)), 8);
            if (this._blank != null)
                return;
        ...
        public void DoDraw(GraphicsDevice device)
        {
            ...
            int num1 = 0;
            int num2 = 0;
            this.DrawLine(p1_1, new Vec2(p1_1.x + (float)this._packetsToDraw * this._widthPerPacket, p1_1.y), Color.White);
            int num3 = 0;
            foreach (NCPacketBreakdown packet in this._packets)
            {
                int num4 = 0;
                Vec2 vec2 = new Vec2(p1_1.x + (float)num1 * this._widthPerPacket, p1_1.y + this._maxHeightPerPacket);
                foreach (NCPacketDataType dataType in NCPacketBreakdown.dataTypes)
                {
                    ...
                    if (num5 > 0)
                    {
                        ...
                        Vec2 p1_2 = vec2 - new Vec2(0.0f, (float)num4 / (float)this._currentMaxSize * this._maxHeightPerPacket);
                        Vec2 p2 = p1_2 - new Vec2(0.0f, (float)num5 / (float)this._currentMaxSize * this._maxHeightPerPacket);
            ...
            NetGraph._biosFont.Draw(this.batch, "in: " + (this._currentMaxSize / 8).ToString() + " bytes - " + ((int)((double)num3 / (double)this._packets.Count / 8.0)).ToString() + " avg", p1_1.x, (float)((double)p1_1.y - 9.0), Color.White);
            NetGraph._biosFont.Draw(this.batch, "fps: " + this.fps.ToString() + "    ping: " + ((int)((double)this.ping * 1000.0)).ToString() + "ms", p1_1.x, (float)((double)p1_1.y + (double)this._maxHeightPerPacket + 8.0), Color.White);
        }
        ...
        public void DrawChart(Vec2 pos)
        {
            ...
            {
                int num4 = 0;
                Vec2 vec2 = new Vec2(pos.x + (float)num1 * this._widthPerPacket, pos.y + this._maxHeightPerPacket);
                foreach (NCPacketDataType dataType in NCPacketBreakdown.dataTypes)
                {
                    int num5 = packet.Get(dataType);
                    if (num5 > 0)
                    {
                        ...
                        DuckGame.Graphics.DrawLine(p1, p1 - new Vec2(0.0f, (float)num5 / (float)this._currentMaxSize * this._maxHeightPerPacket), typeColor, this._widthPerPacket, (Depth)0.9f);
                        num4 += num5;
                        num3 += num5;
                    }
                }
            ...
            {
                NetGraph.avgBytes = (float)NetGraph.numBytes / (float)NetGraph.numFrames;
                NetGraph.avgHeaderBytes = (float)NetGraph.numHeaderBytes / (float)NetGraph.numFrames;
                NetGraph.avgGhostBytes = (float)NetGraph.numGhostBytes / (float)NetGraph.numFrames;
                NetGraph.avgAckBytes = (float)NetGraph.numAckBytes / (float)NetGraph.numFrames;
                NetGraph.numBytes = 0;
        ...
        '~'
            ...
            public BFont(Texture2D image, int size)
            {
                ...
                for (int index1 = 0; index1 < (int)byte.MaxValue; ++index1)
                {
                    ...
                    {
                        if ((int)NetGraph.BFont._characters[index2] == (int)ch)
                        {
            ...
            public void Draw(SpriteBatch batch, string text, float xpos, float ypos, Color c)
            {
                ...
                for (int index = 0; index < text.Length; ++index)
                {
                    if (text[index] == '\n')
                    {
                        num1 += (float)(this._tileSize * 8);
                        num2 = 0.0f;
                    }
                    else
                    {
                        int character = NetGraph.BFont._characterMap[(int)text[index]];
                        int x = character % 16 * this._tileSize;
                        int y = character / 16 * this._tileSize;
                        batch.Draw(this._texture, (Vector2)new Vec2(xpos + num2, ypos), new Microsoft.Xna.Framework.Rectangle?((Microsoft.Xna.Framework.Rectangle)new Rectangle((float)x, (float)y, (float)this._tileSize, (float)this._tileSize)), (Microsoft.Xna.Framework.Color)c, 0.0f, (Vector2)Vec2.Zero, (Vector2)new Vec2(1f, 1f), SpriteEffects.None, 1f);
                        num2 += (float)this._tileSize;
                    }
                }
            }
        }
