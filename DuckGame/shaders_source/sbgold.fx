...
float4 PixelShaderFunction(float2 uv: TEXCOORD0, float4 c: COLOR0) : COLOR0
{
    ...
    float4 goldCol = sampleAtlasWrap(float2(((realuv.x % width) / width) + (xpos * 0.01f), ((realuv.y % height) / height) + (ypos * 0.01f)), float2(goldxoffset, goldyoffset), float2(goldsizey, goldsizex));
