...
namespace DuckGame
{
    public class NetGraph
    {
        ...
        public NetGraph() => _font = new BitmapFont("biosFont", 8);
        ...
        public void DrawLine(Vec2 p1, Vec2 p2, Color col, float width = 1f)
        {
            ...
            batch.Draw(_blank, (Vector2)p1, new Microsoft.Xna.Framework.Rectangle?(), (Microsoft.Xna.Framework.Color)col, rotation, (Vector2)new Vec2(0f, 0.5f), (Vector2)new Vec2(length, width), SpriteEffects.None, 1f);
        }
        ...
        public void InitializeDraw(GraphicsDevice device)
        {
            ...
            if (_blank != null)
                return;
            _blank = new Texture2D(device, 1, 1, false, SurfaceFormat.Color);
            _blank.SetData<Color>(new Color[1]
            {
        Color.White
            });
        }
        ...
        public void DoDraw(GraphicsDevice device)
        {
            ...
            int num1 = 0;
            int num2 = 0;
            DrawLine(p1_1, new Vec2(p1_1.x + _packetsToDraw * _widthPerPacket, p1_1.y), Color.White);
            int num3 = 0;
            foreach (NCPacketBreakdown packet in _packets)
            {
                int num4 = 0;
                Vec2 vec2 = new Vec2(p1_1.x + num1 * _widthPerPacket, p1_1.y + _maxHeightPerPacket);
                foreach (NCPacketDataType dataType in NCPacketBreakdown.dataTypes)
                {
                    ...
                    if (num5 > 0)
                    {
                        ...
                        Vec2 p1_2 = vec2 - new Vec2(0f, num4 / (float)_currentMaxSize * _maxHeightPerPacket);
                        Vec2 p2 = p1_2 - new Vec2(0f, num5 / (float)_currentMaxSize * _maxHeightPerPacket);
                        DrawLine(p1_2, p2, typeColor, _widthPerPacket);
                        num4 += num5;
                        num3 += num5;
                    }
                }
            ...
            if (num2 >= _currentMaxSize)
                _currentMaxSize = num2;
            else
                _currentMaxSize -= 2;
            NetGraph._biosFont.Draw(batch, "in: " + (_currentMaxSize / 8).ToString() + " bytes - " + ((int)(num3 / _packets.Count / 8.0)).ToString() + " avg", p1_1.x, (float)(p1_1.y - 9.0), Color.White);
            NetGraph._biosFont.Draw(batch, "fps: " + fps.ToString() + "    ping: " + ((int)(ping * 1000.0)).ToString() + "ms", p1_1.x, (float)(p1_1.y + _maxHeightPerPacket + 8.0), Color.White);
        }
        ...
        public void DrawChart(Vec2 pos)
        {
            if (!(Network.activeNetwork.core is NCBasic))
                return;
            if (!_drawInitialized)
                InitializeDraw(DuckGame.Graphics.device);
            while (_packets.Count > _packetsToDraw)
                _packets.Dequeue();
            int num1 = 0;
            int num2 = 0;
            DuckGame.Graphics.DrawLine(pos, new Vec2(pos.x + _packetsToDraw * _widthPerPacket, pos.y), Color.White, depth: ((Depth)0.9f));
            int num3 = 0;
            foreach (NCPacketBreakdown packet in _packets)
            {
                int num4 = 0;
                Vec2 vec2 = new Vec2(pos.x + num1 * _widthPerPacket, pos.y + _maxHeightPerPacket);
                foreach (NCPacketDataType dataType in NCPacketBreakdown.dataTypes)
                {
                    int num5 = packet.Get(dataType);
                    if (num5 > 0)
                    {
                        Color typeColor = NCPacketBreakdown.GetTypeColor(dataType);
                        Vec2 p1 = vec2 - new Vec2(0f, num4 / (float)_currentMaxSize * _maxHeightPerPacket);
                        DuckGame.Graphics.DrawLine(p1, p1 - new Vec2(0f, num5 / (float)_currentMaxSize * _maxHeightPerPacket), typeColor, _widthPerPacket, (Depth)0.9f);
                        num4 += num5;
                        num3 += num5;
                    }
                }
            ...
            DuckGame.Graphics.DrawString("in: " + (_currentMaxSize / 8).ToString() + " bytes - " + ((int)(num3 / _packets.Count / 8.0)).ToString() + " avg", new Vec2(pos.x, pos.y - 9f), Color.White, (Depth)0.9f);
            DuckGame.Graphics.DrawString("fps: " + fps.ToString() + "    ping: " + ((int)(ping * 1000.0)).ToString() + "ms     bytes out:" + NetGraph.avgBytes.ToString("0.00") + " hdr:" + NetGraph.avgHeaderBytes.ToString("0.00") + " ack:" + NetGraph.avgAckBytes.ToString("0.00") + " gst:" + NetGraph.avgGhostBytes.ToString("0.00"), new Vec2(pos.x, (float)(pos.y + _maxHeightPerPacket + 8.0)), Color.White, (Depth)0.9f);
        }
