...
namespace DuckGame
{
    public static class Collision
    {
        ...
        public static bool Point(Vec2 point, Thing t) => point.x >= (double)t.left && point.x <= (double)t.right && point.y >= (double)t.top && point.y <= (double)t.bottom;

        public static bool Point(Vec2 point, Rectangle r) => point.x >= (double)r.Left && point.x <= (double)r.Right && point.y >= (double)r.Top && point.y <= (double)r.Bottom;
        ...
        public static bool Line(Vec2 point1, Vec2 point2, Thing t)
        {
            ...
            double num5 = point2.x - (double)point1.x;
            if (Math.Abs(num5) > 1E-07)
            {
                double num6 = (point2.y - (double)point1.y) / num5;
        ...
        public static bool Line(Vec2 point1, Vec2 point2, Rectangle rect)
        {
            double x = rect.x;
            double y = rect.y;
            double num1 = rect.x + (double)rect.width;
            double num2 = rect.y + (double)rect.height;
            ...
            double num7 = point2.x - (double)point1.x;
            if (Math.Abs(num7) > 1E-07)
            {
                double num8 = (point2.y - (double)point1.y) / num7;
        ...
        public static bool CCW(Vec2 A, Vec2 B, Vec2 C) => (C.y - (double)A.y) * (B.x - (double)A.x) > (B.y - (double)A.y) * (C.x - (double)A.x);
        ...
        public static bool Circle(Vec2 center, float radius, Thing t)
        {
            ...
            if (center.x < (double)t.left)
                vec2_1.x = t.left;
            else if (center.x > (double)t.right)
                vec2_1.x = t.right;
            if (center.y < (double)t.top)
                vec2_1.y = t.top;
            else if (center.y > (double)t.bottom)
                vec2_1.y = t.bottom;
            Vec2 vec2_2 = vec2_1 - center;
            return vec2_2.x * (double)vec2_2.x + vec2_2.y * (double)vec2_2.y <= (double)radius * (double)radius;
        }
        ...
        public static bool Rect(Vec2 tl1, Vec2 br1, Thing t) => br1.y >= (double)t.top && tl1.y <= (double)t.bottom && tl1.x <= (double)t.right && br1.x >= (double)t.left;

        public static bool Rect(Vec2 tl1, Vec2 br1, Rectangle t) => br1.y >= (double)t.y && tl1.y <= (double)t.Bottom && tl1.x <= (double)t.Right && br1.x >= (double)t.x;

        public static bool Rect(Rectangle r1, Rectangle r2) => r1.y + (double)r1.height >= r2.y && r1.y <= r2.y + (double)r2.height && r1.x <= r2.x + (double)r2.width && r1.x + (double)r1.width >= r2.x;

        public static bool Rect(Rectangle r1, Vec4 r2) => r1.y + (double)r1.height >= r2.y && r1.y <= r2.y + (double)r2.w && r1.x <= r2.x + (double)r2.z && r1.x + (double)r1.width >= r2.x;
          ...
          Vec2 line2V2)
        {
            ...
            float num3 = (float)((double)num1 * line1V1.x + (double)num2 * line1V1.y);
            float num4 = line2V2.y - line2V1.y;
            float num5 = line2V1.x - line2V2.x;
            float num6 = (float)((double)num4 * line2V1.x + (double)num5 * line2V1.y);
        ...
        thing.bottom - point1.y
            ...
            for (int index = 0; index < 4; ++index)
            {
                ...
                else
                {
                    ...
                    if (numArray1[index] < 0.0 && (double)num1 < (double)num3)
                        num1 = num3;
                    else if (numArray1[index] > 0.0 && (double)num2 > (double)num3)
                        num2 = num3;
                }
            }
