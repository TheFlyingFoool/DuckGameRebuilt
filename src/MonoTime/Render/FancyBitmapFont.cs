    ...
    public class FancyBitmapFont : Transform
    {
      ...
      '¿'
        ...
        public static void InitializeKanjis()
        {
            ...
            FancyBitmapFont._kanjiMap = new ushort[(int)ushort.MaxValue];
            for (int index = 0; index < str.Length; ++index)
                FancyBitmapFont._kanjiMap[(int)str[index]] = (ushort)index;
        }
        ...
        protected void Construct(string image)
        {
            ...
            if (!FancyBitmapFont.widthMap.TryGetValue(image, out this._widths))
            {
                ...
                for (int y = 1; y < this._texture.height; y += this._charHeight + 1)
                {
                    ...
                    {
                        if (data[index1 + y * this._texture.width].r == (byte)0 && data[index1 + y * this._texture.width].g == (byte)0 && data[index1 + y * this._texture.width].b == (byte)0 && data[index1 + y * this._texture.width].a == (byte)0)
                        {
                            if (x == -1)
                                x = index1;
                        }
                        else if (x != -1)
                        {
                            if (this._charHeight == 0)
                            {
                                ...
                                {
                                    if (data[num + index2 * this._texture.width].r != (byte)0 || data[num + index2 * this._texture.width].g != (byte)0 || data[num + index2 * this._texture.width].b != (byte)0 || data[num + index2 * this._texture.width].a != (byte)0)
                                    {
                            ...
                            this._widths.Add(new Rectangle((float)x, (float)y, (float)(index1 - x), (float)this._charHeight));
                            x = -1;
                        }
                    }
                    if (flag)
        ...
        public string FormatWithNewlines(string pText, float maxWidth, bool thinButtons = false)
        {
            ...
            for (this._letterIndex = 0; this._letterIndex < charArray.Length; ++this._letterIndex)
            {
                ...
                if (charArray[this._letterIndex] == '@' || this.chatFont && charArray[this._letterIndex] == ':')
                {
                    ...
                    Sprite sprite = this.ParseSprite(pText, (InputProfile)null);
                    if (sprite != null)
                    {
                ...
                if (!flag)
                {
                    ...
                    {
                        int character = FancyBitmapFont._characterMap[(int)index];
        ...
        public float GetWidth(string text, bool thinButtons = false)
        {
            ...
            for (this._letterIndex = 0; this._letterIndex < text.Length; ++this._letterIndex)
            {
                ...
                {
                    int letterIndex = this._letterIndex;
                    Sprite sprite1 = this.ParseSprite(text, (InputProfile)null);
                    if (sprite1 != null)
                    {
                        if (this.chatFont)
                        {
                            ...
                            sprite2.scale = sprite2.scale * (this.scale.x / 2f);
                            if (this is RasterFont)
                            {
                                float num2 = (float)((double)(this as RasterFont).data.fontSize * (double)RasterFont.fontScaleFactor / 10.0);
                                Sprite sprite3 = sprite1;
                                sprite3.scale = sprite3.scale * num2;
                                sprite1.scale = new Vec2((float)Math.Round((double)sprite1.scale.x * 2.0) / 2f);
                            }
                            num1 += (float)((double)sprite1.width * (double)sprite1.scale.x + 1.0);
                            sprite1.scale = scale;
                        }
                        else
                            num1 += thinButtons ? 6f : (float)((double)sprite1.width * (double)sprite1.scale.x + 1.0);
                        flag = true;
                    }
                    else
                        this._letterIndex = letterIndex;
                ...
                if (!flag)
                {
                    ...
                    {
                        int index2 = FancyBitmapFont._characterMap[(int)index1];
                ...
                if (!flag1)
                {
                    ...
                    if (this.maxWidth > 0)
                    {
                        if (text[this._letterIndex] == ' ' || text[this._letterIndex] == '|' || text[this._letterIndex] == '@')
                        {
                            ...
                            {
                                char index2 = (char)Maths.Clamp((int)text[index1], 0, FancyBitmapFont._characterMap.Length - 1);
                                int character = FancyBitmapFont._characterMap[(int)index2];
                                if (this._characterInfos != null)
                                {
                        ...
                        {
                            char index = (char)Maths.Clamp((int)text[this._letterIndex], 0, FancyBitmapFont._characterMap.Length - 1);
                            int character = FancyBitmapFont._characterMap[(int)index];
                            if (this._characterInfos != null)
                            {
                                ...
                                if ((double)num1 + (double)num6 * (double)this.scale.x > (double)this.maxWidth)
                                {
                                    ++num3;
                                    num2 += (float)this._charHeight * this.scale.y;
                                    num1 = 0.0f;
            ...
            for (this._letterIndex = 0; this._letterIndex < text.Length; ++this._letterIndex)
            {
                ...
                if (!flag1)
                {
                    ...
                    if (this.maxWidth > 0)
                    {
                        ...
                        {
                            char index3 = (char)Maths.Clamp((int)text[this._letterIndex], 0, FancyBitmapFont._characterMap.Length - 1);
                            Rectangle width = this._widths[FancyBitmapFont._characterMap[(int)index3]];
                    ...
                    if (!flag2)
                    {
                        ...
                        {
                            char index4 = (char)Maths.Clamp((int)text[this._letterIndex], 0, FancyBitmapFont._characterMap.Length - 1);
                            int character = FancyBitmapFont._characterMap[(int)index4];
                            if (this._characterInfos != null)
                            {
                                if (character < this._characterInfos.Count)
            ...
            for (this._letterIndex = 0; this._letterIndex < text.Length; ++this._letterIndex)
            {
                ...
                if (text[this._letterIndex] == '@' || this.chatFont && text[this._letterIndex] == ':')
                {
                    ...
                    if (sprite1 != null)
                    {
                        ...
                        if (sprite1 != null)
                        {
                            float num4 = (float)(this.characterHeight / 2 - sprite1.height / 2) + this.symbolYOffset;
                            if (colorSymbols)
                                sprite1.color = c;
                            if (this.chatFont)
                            {
                                ...
                                if (this is RasterFont)
                                {
                                    float num5 = (float)((double)(this as RasterFont).data.fontSize * (double)RasterFont.fontScaleFactor / 10.0);
                                    Sprite sprite3 = sprite1;
                                    sprite3.scale = sprite3.scale * num5;
                                    sprite1.scale = new Vec2((float)Math.Round((double)sprite1.scale.x * 2.0) / 2f);
                                }
                                float num6 = (float)((double)this.characterHeight * (double)this.scale.y / 2.0 - (double)sprite1.height * (double)sprite1.scale.y / 2.0);
                            ...
                            else if (this._rasterData != null)
                            {
                                ...
                                sprite4.scale = sprite4.scale * num7;
                ...
                if (!flag1)
                {
                    ...
                    if (this.maxWidth > 0)
                    {
                        if (text[this._letterIndex] == ' ' || text[this._letterIndex] == '|' || text[this._letterIndex] == '@')
                        {
                            ...
                            {
                                char index2 = ' ';
                                float width1 = this._widths[FancyBitmapFont._characterMap[(int)(byte)index2]].width;
                                for (; index1 < text.Count<char>() && text[index1] != ' ' && text[index1] != '|' && text[index1] != '@'; ++index1)
                                {
                                    byte index3 = (byte)Maths.Clamp((int)text[index1], 0, 254);
                                    Rectangle width2 = this._widths[FancyBitmapFont._characterMap[(int)index3]];
                        ...
                        {
                            byte index = (byte)Maths.Clamp((int)text[this._letterIndex], 0, 254);
                            Rectangle width = this._widths[FancyBitmapFont._characterMap[(int)index]];
                    ...
                    if (!flag2)
                    {
                        ...
                        {
                            num1 += ((float)this._charHeight + this.lineGap) * this.scale.y;
                            num2 = 0.0f;
                            ++num3;
                        }
                        else
                        {
                            ...
                            if (index4 >= 'ぁ')
                            {
                                int kanji = (int)FancyBitmapFont._kanjiMap[(int)index4];
                                ...
                                Graphics.Draw((Sprite)FancyBitmapFont._kanjiSprite, (float)((double)xpos + (double)num2 + 1.0), (float)((double)ypos + (double)num1 + 1.0), deep);
                                num2 += 8f * this.scale.x;
                            }
                            else
                            {
                                int index5 = FancyBitmapFont._characterMap[(int)index4];
            ...
            for (this._letterIndex = 0; this._letterIndex < text.Length; ++this._letterIndex)
            {
                ...
                if (text[this._letterIndex] == '|')
                {
                    ...
                    if (color2 != Colors.Transparent)
                    {
                        ...
                        text1 = "";
                        richTextBox.SelectionColor = System.Drawing.Color.FromArgb((int)color2.r, (int)color2.g, (int)color2.b);
                        flag = true;
                    }
                    else
                        this._letterIndex = letterIndex;
